...
function getQuestionInfo($openid, $answer)
{
	...
	if (!$con){
		//$log->Debug('Could not connect: ' . mysql_error());
    ...
    $finish_question = 0;
    if ($answer == "答题"){
        $mysql_insert = "INSERT INTO `".$mysql_table_record."` (`openid`, `question`, `question_y`, `question_n`) VALUES('".
        	$openid."', '0', '0', '0') ON DUPLICATE KEY UPDATE `question` = '0', `question_y` = '0', `question_n` = '0';";
        ...
        if(!$row){
        	return "抱歉！\n你输入的字符被系统设别为答题选项，如果你想答题，请输入“答题”。";
        }
    ...
    if ($finish_question == 0){
		...
		while ( $row = mysql_fetch_array ( $result ) ) {
			if ($first) {
				...
				$result_yesno = mysql_query ( $mysql_update_yesno );
				// $log->Debug('sql:'.$mysql_update_yesno);
				// $log->Debug('$result_yesno:'.$result_yesno);
			} else {
				$content .= "第" . $row ['id'] . "题：" . $row ['question'] . "\n";
				$content .= empty ( $row ['optionA'] ) ? "" : "A. " . $row ['optionA'] . "\n";
				$content .= empty ( $row ['optionB'] ) ? "" : "B. " . $row ['optionB'] . "\n";
